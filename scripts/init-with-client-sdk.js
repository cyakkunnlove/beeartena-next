...
async function initializeFirebaseData() {
  try {
    ...
    let adminUser;
    try {
      const userCredential = await createUserWithEmailAndPassword(auth, 'admin@beeartena.com', 'BeeArtEna2024Admin!');
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        // 既存ユーザーでログイン
        const userCredential = await signInWithEmailAndPassword(auth, 'admin@beeartena.com', 'BeeArtEna2024Admin!');
        adminUser = userCredential.user;
      } else {
        throw error;
      }
    ...
    try {
      const testCredential = await createUserWithEmailAndPassword(auth, 'test@example.com', 'testpass123');

...
async function verifyAdminCustomerInteraction() {
  ...
  try {
    ...
    // 3. ポイント履歴
    console.log('\n=== 3. ポイント履歴 ===');
    const pointHistory = await db.collection('pointTransactions')
    ...
    // その日の予約状況
    const tomorrowReservations = await db.collection('reservations')
      .where('date', '==', dateStr)
    ...
    // 管理者が作成した予約
    const adminCreatedReservations = await db.collection('reservations')

...
async function verifyAdminSimple() {
  ...
  try {
    ...
    console.log(`- 住所: ${customerData.address ? `${customerData.address.prefecture} ${customerData.address.city}` : '未設定'}`);
    ...
    activeServices.forEach(service => {
      const vipPrice = customerData.vipStatus && service.price > 30000 
        ? Math.floor(service.price * 0.95) // VIP5%割引
        : service.price;
        
      console.log(`- ${service.name}: ¥${service.price.toLocaleString()}${
        customerData.vipStatus && service.price > 30000 ? ` → VIP価格: ¥${vipPrice.toLocaleString()}` : ''

...
// ユーザー権限チェック（本人または管理者）
export async function requireUserOrAdmin(request: NextRequest, targetUserId: string): Promise<NextResponse | null> {
  const user = await verifyAuth(request);
...
export function rateLimit(request: NextRequest, limit: number = 100, windowMs: number = 60000): NextResponse | null {

...
const normalizeServicePlanRecord = (record: AdminServicePlanRecord): ServicePlan => {
  ...
  return {
    ...
    monitorEnabled: Boolean(record.monitorEnabled),
    otherShopPrice: toOptionalNumberValue(record.otherShopPrice ?? undefined),
    campaignPrice: toOptionalNumberValue((record as any).campaignPrice ?? undefined),
    campaignReferralDiscount: toOptionalNumberValue((record as any).campaignReferralDiscount ?? undefined),
    secondPrice: toOptionalNumberValue((record as any).secondPrice ?? undefined),
    retouchPrice3m: toOptionalNumberValue((record as any).retouchPrice3m ?? undefined),
    retouchPrice6m: toOptionalNumberValue((record as any).retouchPrice6m ?? undefined),
    durationText: typeof (record as any).durationText === 'string' ? (record as any).durationText : undefined,
    note: typeof (record as any).note === 'string' ? (record as any).note : undefined,
